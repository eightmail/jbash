...
**jbash** integrates local LLMs (such as Qwen 2.5) with local shell command execution. **Use this tool with extreme caution.** 
...
Running AI-generated commands directly on your local system carries inherent risks:

* **Unpredictable LLM Behavior:** Large Language Models are probabilistic and prone to hallucinations, misinterpretations, or edge-case logic failures. A flawed command generated by the model can result in accidental data loss or system corruption (e.g., unintended file deletions or incorrect path expansions).
* **Prompt Injection Vulnerabilities:** If `jbash` processes external text inputs—such as untrusted READMEs, malicious Git commit messages, or error logs—an attacker could use indirect prompt injection to trick the LLM into executing unauthorized commands.
...
spinner with the active model name plus a live token count**: the spinning
glyph cycles through katakana on the default theme, neon green:
...
boundary: jbash runs without privileges and (on common hardened hosts, this
one included) without unprivileged user namespaces, so a command that knows an
...
- Add some features to improve security and mitigate the inherent dangers imposed by Remote Code Execution (RCE) / Prompt Injection.
