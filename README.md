...
## ⚠️ Security Warning & Risk Disclosure
...
jbash --sandbox              force sandboxed AI tool execution (on by default)
jbash --insecure             disable the sandbox for AI tool commands
```
...
~/.jbash/sandbox       session sandbox mode: '1' sandboxed (default) | '0' insecure
...
- `jbash --sandbox` starts (or re-affirms) the sandboxed mode. This is
  already the default, so the flag mainly exists for clarity and for tools
  that want to be explicit.
- `jbash --insecure` disables the sandbox: tool commands run with your real
  `$HOME` and environment, exactly like the confirmed-command path. A
  warning is printed at startup, and `~/.jbash/sandbox` records the session's
  mode so the shell's `ai`/`ask`/`fix` wrappers re-apply it on every call.
